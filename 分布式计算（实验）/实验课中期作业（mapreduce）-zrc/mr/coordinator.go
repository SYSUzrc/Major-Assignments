...
var maptasks chan string // Map任务的通道
  ...
  func checkAllMap(m *Coordinator) bool {
      m.RWLock.RLock()
 ...
 func checkReduce(m *Coordinator) bool {
     m.RWLock.RLock()
     defer m.RWLock.RUnlock()
...
func (m *Coordinator) RPCHandler(args *MyArgs, reply *MyReply) error {
	     ...
	     case Msg_AskingTask: //发送任务，Map全部执行完之前不会发送Reduce
	         ...
	         case filename := <-maptasks:
	             ...
	             reply.MapNum = m.MapArray[m.MapTaskNum]
	             reply.NReduce = m.NReduce
                reply.TaskType = "map"
                m.RWLock.Lock()
                m.FilesName[filename] = Allocated
                var i int = m.MapTaskNum
                m.MapTaskNum++
                m.RWLock.Unlock()
                go m.timerForWorker("map", filename, i)
                return nil
            case reduceNum := <-reducetasks:
                ...
                m.RWLock.Unlock()
                go m.timerForWorker("reduce", strconv.Itoa(reduceNum), 0)
                return nil
            }
